...
animo a decir que un amigo desde los inicios de Akua
construimos la plataforma de desarrollo interna, ambos de experiencias
e industrias diferentes pero con una misma visión profunda sobre lo
que necesitaba Akua.

Deje enlace en su linkedin, pero a muy breve --e irrespetuoso resumen--
Ger viene con una expriencia desde el mas bajo nivel en infraestructura
a llegar a constuir el equipo y la plataforma en Sate (para los amigos),
...
en Akua, una cuenta de AWS pelada pero eso era lo que menos nos proecupaba;
...
ciclo de vida por la que pase el software.
...
hablamos de tres, desde el inicio descartamos una y con las dos restantes
...
por Pulumi, en nuestro caso si no recuerdo mal tuvimos que hacerlo en
...
en contraparte mi línea de pensamiento era que los desarrolladores de Akua
no íban a sentirse cómodos teniendo que saber manejar una herramienta
de plataforma, debíamos ofrecer una capa de presentacion --aka portal--
y que desde ahí puedan diseñar la arquitectura del proyecto y posterior
despliegue y gestión.

Acá al igual que nuestra herramienta de backend hoy en día aparecen
dos posibilidades Backstage o Port.io, en realidad hay mas, pero como
siempre buscamos las de mayor adopción en la industria para que el día de mañana
quien ingrese al equipo no se encuentre con algo que no se usa en ningún lado.
...
la UI es exquisita, brinda acceso por SSO, con sistema de RBAC (aun bastante mejorable)
...
de presentación de nuestra Plataforma.

### Stack tecnológico

Desarrollamos un Helm chart a medida para nuestras aplicaciones, con el tiempo
evolucionó y hoy soporta poder desplegar un microservicio o un monorepo con esto
último nos queremos referir a que en un mismo proyecto se puedan desplegar múltiples
servicios en kubernetes sin necesidad de tener "n" proyectos en Gitlab.

Nuestro helm chart (también pensado desde el día cero) es único y
...
nuestro helm-chart es único y cada proyecto definie sus values.yaml que en tiempo
...
version de kubernetes es trivial, es PCI compliance, entre otros.

Tiene sus contras como todo, no se pueden instalar herramienteas en kubernetes
que necesiten desplegar un daemonset, igual mucho no nos preocupa eso porque
(volviendo a nuestros mantras de mantener todo lo mas simple posible),
...
y disponibilizar los secretos a las aplicaciones con la flexibilidad de que si
algun secreto no es manejado por nuestra infra-lib el equipo de seguridad pueda
agregar parameter store y que queden disponibles para las aplicaciones.


#### Seguridad auth y authz?

Desde el minuto cero para las aplicaciones usamos IRSA (Iam Role for Service Account)
desplegando permisos por tag (en breve comento nuestra estrategia de tagging),
y en caso  de que el recurso de AWS no lo soporte (s3), nuestra infra-lib tiene
la habilidad de autodescubrir el recurso y asignar los permisos necesiarios.
...
sea todo mas facil al tener nuestro recursos correctamente "taggeados" y dejo una
breve PD: el gasto de nuestra infraestructura es bastante mas bajo del que habíamos
predicho gracias a las correctas configuraciones por entorno de nuestra infra-lib.
...
acciones del dia 2, despliegue de single tenants, pero lo guardo para otra
publicación :).
...
nada que ver, como les contaba constuimos un flujo de laburo donde
diseñamos, evaluamos multiples aleternativas y casuísticas y
finalmente implementamos; al final del camino es realmente un trabajo
en equipo donde lo que prevalece es nuestra plataforma y
...
construimos y que es para toda la vida!
