...
con el objetivo de construir la plataforma de desarrollo interna,
...
Ger viene con una experiencia desde el más bajo nivel en infraestructura
hasta llegar a construir el equipo y la plataforma en Sate (para los amigos),
...
dejo un punteo de "nuestros mantras" que hasta el día de hoy seguimos
...
mata por nuestra forma de trabajo.
...
futuro si sumamos a más compas al equipo no se van a encontrar con algo totalmente
desconocido; por último y no menor te ofrece la posibilidad de implementar
dynamic resources osea si hay algún recurso que no este disponible en Pulumi
...
dos oportunidades con Typesense y "marcas de deployment" en NewRelic.
...
fase de definición de nuestro IDP, la capa de presentación.

Como dice el título acá pudimos debatir desde las diferencias que íbamos a
ofrecer, Ger desde su experiencia con equipos extremadamente técnicos y que
...
de presentación --aka portal-- y que desde ahí puedan diseñar la
...
hay más claro, pero como siempre buscamos las de mayor adopción en la
...
estábamos no queríamos tocar nada de frontend, no es nuestro fuerte y nos
obligaba a tener un desvío de tiempos importante que no eran una opción.

En el caso de Port.io no fue por descarte, me gusta hablar de Port como
el Notion de las plataformas, su sistema de Blueprints te permite diseñar
...
de presentación de nuestra Plataforma que estábamos buscando.
...
Acá una vez más nuestro enfoque fue, centralicemos la herramienta y la evolución,
intentando no transferir carga cognitiva a nuestros clientes; así que en este punto
nuestro helm-chart evoluciona y cada proyecto define sus values.yaml que en tiempo
...
por un esquema de pipelines centralizados y que los proyectos solo hagan el "include"
y no se preocupen de nada más, en caso de necesitar algo se agrega al pipeline centralizado
...
Tiene sus contras como todo, no se pueden instalar herramientas en kubernetes
que necesitan desplegar un daemonset, igual mucho no nos preocupa eso porque
(volviendo a uno de nuestros mantras de mantener todo lo más simple posible),
...
más herramientas en nuestros clusters.

#### Un momento y los secretos?

Desarrollamos un flujo de trabajo con parameter store que nos permite desplegar
y disponibilizar los secretos a las aplicaciones en kubernetes con la flexibilidad
de que si algún secreto no es manejado por nuestra infra-lib el equipo de seguridad
...
sea todo más facil al tener nuestro recursos correctamente "taggeados" y le dejo una
breve PD: el gasto de nuestra infraestructura es bastante más bajo del que habíamos
...
es mediante reglas en los security groups entre la app y la DB, así mismo el acceso
entre aplicaciones está bloqueado a nivel del red aunque estén el mismo namespace,
...
Resta aún comentar un monton de cosas más: como implementamos scorecards,
acciones del día 2, despliegue de single tenants, publicación de rutas públicas;
...
de cada uno; construimos un flujo de trabajo que nos llevó a
...
poco importa sinceramente, como les contaba construimos un flujo de
laburo donde diseñamos y evaluamos multiples alternativas y casuísticas
...
juntos esta plataforma y lo más importante la amistad que
